...
tcProc :: InPat Name -> LHsCmdTop Name		-- proc pat -> expr
       ...
       -> TcM (OutPat TcId, LHsCmdTop TcId, HsCoWrapper)

tcProc pat cmd exp_ty
  = newArrowScope $
    do	{ (mk_co, (exp_ty1, res_ty)) <- matchExpectedAppTy exp_ty 
	; (mk_co1, (arr_ty, arg_ty)) <- matchExpectedAppTy exp_ty1
	; let cmd_env = CmdEnv { cmd_arr = arr_ty }
        ; (pat', cmd') <- tcPat ProcExpr pat arg_ty $
			  tcCmdTop cmd_env cmd [] res_ty
        ; let mk_res_co = joinMk (\co co1 -> mkTransCo co (mkAppCo co1 (mkReflCo res_ty))) mk_co mk_co1
        ; made_res_co <- finaliseMk mk_res_co
        ; return (pat', cmd', made_res_co) }
\end{code}
